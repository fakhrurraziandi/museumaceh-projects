...
<h5 style="text-align: center; font-weight: bold; margin-bottom: 0px;">
    LAPORAN PENDAPATAN MUSEUM ACEH
    <br>
    Tanggal {{date('d-m-Y', strtotime($start))}} s/d {{date('d-m-Y', strtotime($end))}}
</h5>

<p style="font-size: 80%; text-indent: 50px;">Dengan ini kami sampaikan data laporan pendapatan UPTD Museum Aceh Tanggal {{date('d-m-Y', strtotime($start))}} s/d {{date('d-m-Y', strtotime($end))}}, yang di unduh dari aplikasi e-ticket Museum Aceh.</p>
    ...
    <thead>
        <tr>
            ...
            @foreach($list_kategori_pengunjung as $kategori_pengunjung)
                ...
                @endif
            @endforeach
            <th style="width: 100px;">Jumlah/Per Hari</th>
        </tr>
    </thead>
            ...
            <tr>
                ...
                @foreach($list_kategori_pengunjung as $kategori_pengunjung)
                    <td>
                        <?php  
                        $sql = "SELECT
                                    SUM(ticket.tarif) as jumlah_pendapatan
                                FROM
                                    ticket
                        ...
                        echo $result[0]->jumlah_pendapatan ? number_format($result[0]->jumlah_pendapatan, 0, ',', '.') : '-';
                        ?>
                    </td>
                    @if($kategori_pengunjung->enable_rombongan)
                        <td>
...
<p style="font-size: 80%; text-indent: 50px;">Laporan pendapatan UPTD Museum Aceh ini adalah laporan resmi berdasarkan data pendapatan Tanggal {{date('d-m-Y', strtotime($start))}} s/d {{date('d-m-Y', strtotime($end))}}. Demikian laporan ini kami sampaikan, agar dapat digunakan sesuai dengan kebutuhan. Terima kasih.   </p>
